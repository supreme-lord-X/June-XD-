...
cmd({
    ...
    try {
        ...
        let madeSetting = `╭───⚙️ *${CONFIG.JUNE XD} Settings* ⚙️───╮
│
│ *Auto Read Status*: ${statusIcon(config.AUTO_READ_STATUS)}
│ *Mode*: *${config.MODE}*
│ *Auto Voice*: ${statusIcon(config.AUTO_VOICE)}
│ *Auto Sticker*: ${statusIcon(config.AUTO_STICKER)}
│ *Auto Reply*: ${statusIcon(config.AUTO_REPLY)}
│ *Alive Message*: *${config.ALIVE_MSG}*
